...
class Filtr():
    ...
    def process(self, path = None,workers = os.cpu_count()):
        ...
        num_cpu = workers
        processes = []
        q = self.get_queues(workers) if path is None else self.get_queues(path = path, workers = workers)
    ...
    def get_queues(self,path = None,workers = os.cpu_count()):

        q = queue.Queue()
        num_cpu = workers
        #print(f"CPUS: {num_cpu}")

        files = self.current_files if path is None else self.files(path = path)
        num_files = len(files)
    ...
    def duration(self,file):
        try:
            ...
            #print(f"The File: {os.path.basename(file)} is an unsupported type and \nits duration could not be calculated.\n")
            duration = 0
        
        return duration

    def generate_data(self,keyword,data_limit = 15,path = None,dest = None,duration_limit = None):
        '''
        The data generated by this function is a list of files found based on the keyword
        entered. It works by starting at 'path' then searches every directory for any file that
        contains the keyword. Once the amount of files founds reaches the data_limit, the function 
        will stop.
        
        parameters
        ---------------
        keyword:
        an identifier to allow the function to know what the filename should contain in order
        to be used as part of the dataset.
                ...
                keyword = 'kick'
                searches for all of the files that has kick in the name

        data_limit:
        default is set to 15 and this is the max number of files to look for before 
        the function will stop searching for files. If the default value isnt changed, the 
        function will stop seeking for files after it has found 15 that matches the keyword.

        path:
        Where the function should begin its search for data

        dest:
        once the files are found, this is where they go        
        '''
        ...
        thispath = self.current_path if path is None else path

        #max length for file
        duration_limit = None if duration_limit is None else duration_limit
        unmoved = []
        try:
            for p,d,files in os.walk(thispath):
                # loops through all files in thispath
                for filename in files:
                    #changes the filename and keyword to lowercase
                    name = filename.lower()
                    keyword = keyword.lower()

                    #creates an absolute path for a file
                    checkpath = os.path.join(p,filename)
                    #exist = os.path.exists(checkpath)

                    #gets the duration of an audio file
                    duration = self.duration(checkpath)
                    
                    # if the files duration is less than the limit it will be moved
                    if duration <= duration_limit:
                        
                        #self.pending is a list of paths for files to transfer
                        #once this list is equal to the data limit the files are moved
                        if len(self.pending) == data_limit:
                            self.xfer(self.pending,dest = dest)
                            print(f"data generated at {dest}")
                            break
                        ...
                        #adds files to the pending list
                            ...
                            if filename.endswith('.wav'):
                                print(f"Adding {filename} to data. \nDuration: {duration}\n")
                        ...
                        if keyword in name and filename.endswith('.wav'):
                            unmoved.append(filename)
                        
                # break once data limit is reached
                if len(self.pending) == data_limit:
                    break
        except:
            pass
        # if the data limit hasnt been reached but os.walk has been exausted, move the files
        if len(self.pending) < data_limit:
            self.xfer(self.pending,dest = dest)

        print(f"There were {len(unmoved)} {keyword} files that exceeded the duration limit and were not moved.\n This includes:\n{unmoved[:5]}")
